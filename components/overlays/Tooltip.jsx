...
export function Tooltip({ label, side = 'top', children }) {
  ...
  return (
    <span style={{ position: 'relative', display: 'inline-flex' }}
      ...
      {children}
      {open ? (
        <span role="tooltip" style={{ position: 'absolute', zIndex: 50, ...pos, whiteSpace: 'nowrap', padding: '5px 9px', background: 'var(--bg-inverse)', color: 'var(--fg-inverse)', fontSize: 'var(--text-xs)', fontFamily: 'var(--font-sans)', borderRadius: 'var(--radius-sm)', boxShadow: 'var(--shadow-2)', pointerEvents: 'none' }}>{label}</span>
      ) : null}
    </span>
  );
}
